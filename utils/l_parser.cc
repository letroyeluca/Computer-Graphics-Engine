    ...
    void parse_rules(std::set<char> const& alphabet, std::map<char, std::vector<std::tuple<double, std::string>>>& rules, stream_parser& parser, bool parse2D)
    {
        ...
        while (true)
        {
            ...
            do {
                ...
                parser.getChar(); // consume the comma
                parser.skip_comments_and_whitespace();

            } while (true);

            // Assign the vector of replacements to the rules map
            rules[alphabet_char] = replacements;
...
std::string const& LParser::LSystem::get_replacement(char c) const
{
    ...
    static thread_local std::string fallback_string;
    auto it = replacementrules.find(c);

    if (it == replacementrules.end() || it->second.empty()) {
        // No replacement rules for this character or empty vector
        fallback_string = c;  // Create a string with just the character
        return fallback_string;
    }

    // If there's only one rule, return it directly
    if (it->second.size() == 1) {
        return std::get<1>(it->second[0]);
    }

    // If there are multiple rules with probabilities, we need to select one based on probability
    double random_value = static_cast<double>(rand()) / RAND_MAX; // Generate a random value between 0 and 1
    double cumulative_probability = 0.0;

    for (const auto& rule_tuple : it->second) {
        double probability = std::get<0>(rule_tuple);
        cumulative_probability += probability;

        if (random_value <= cumulative_probability) {
            return std::get<1>(rule_tuple);
        }
    }

    // If we somehow get here (shouldn't happen if probabilities sum to 1.0)
    // Return the last rule
    return std::get<1>(it->second.back());
}
